import React from 'react'
import {useSelector} from "react-redux";
export default function FooterClinet() {
    const listHeThongRap = useSelector(state => state.RapReducer.heThongRap);
    const renderHeThongRap = () => {
        return listHeThongRap.map((item, index) => {
            return    <a href="#">
                        <img src={item.logo} alt='logo' />
                    </a>
        })
    }
    return (
        <footer className="footer">
            <div className="container">
                <div className="row">
                    <div className="col-2 footer__tix">
                        <h5 className="title_h5">QuDez</h5>
                        <p className="pt-3">FAQ</p>
                        <p>Brand Guidelines</p>
                    </div>
                    <div className="col-2 footer__tt">
                    <h5 className="title_h5">Chính sách</h5>
                        <p>Thỏa thuận sử dụng</p>
                        <p>Chính sách bảo mật</p>
                    </div>
                    <div className="col-4 footer__doitac">
                        <h5 className="title_h5">Đối tác</h5>
                        <div className="footer__icons">
                            <div className="footer__items d-flex">
                              
                                {/* <a href="#">
                                    <img src="./img/bhd.png" alt />
                                </a>
                                <a href="#">
                                    <img src="./img/galaxycine.png" alt />
                                </a>
                                <a href="#">
                                    <img src="./img/cinestar.png" alt />
                                </a>
                                <a href="#">
                                    <img src="./img/404b8c4b80d77732e7426cdb7e24be20.png" alt />
                                </a> */}
                                {renderHeThongRap()}
                            </div>
                        </div>
                    </div>
                    <div className="col-2 footer__mobiles">
                        <h5 className="title_h5">Mobile app</h5>
                        <div className="footer__mobile f-flex pt-3">
                            <a href="#">
                                <img src="data:image/png;base64," />
                            </a>
                            <a href="#">
                                <img src="https://www.apple.com/v/app-store/a/images/overview/icon_appstore__ev0z770zyxoy_large_2x.png" />
                            </a>
                        </div>
                    </div>
                    <div className="col-12 col-lg-2 footer__social">
                        <h5 className="title_h5">Social</h5>
                        <div className="footer__mobile f-flex pt-3">
                            <a href="#">
                                <img src="https://pbs.twimg.com/profile_images/1306051401236099072/nuSA8oqW_400x400.jpg" />
                            </a>
                            <a href="#">
                                <img src="https://cdn.tgdd.vn/2020/03/GameApp/Facebook-200x200.jpg" />
                            </a>
                        </div>
                    </div>
                    <hr />
                    <div className="col-8 footer__text">
                        <p>TIX – SẢN PHẨM CỦA CÔNG TY CỔ PHẦN QuDez</p>
                        <p>Địa chỉ: Quế Thọ, Hiệp Đức, Quảng Nam, Việt Nam.</p>
                        <p>Giấy chứng nhận đăng ký kinh doanh số: 0101659783</p>
                        <p>Số Điện Thoại (Hotline): 1900 545 436</p>
                        <p>Email: <a href="#">quocde99@gmail.com</a></p>
                    </div>
                </div>
            </div>
        </footer>

    )
}
